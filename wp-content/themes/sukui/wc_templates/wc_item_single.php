			...
			<?php if (have_posts()) : the_post(); ?>
				...
				<div <?php post_class() ?> id="post-<?php the_ID(); ?>">
					<div class="storycontent">
						...
						<div id="itempage" class="storesinglebox container">
							<div class="slider">
								<div class="itemimg">
									...
									<div class="itemsubimg">
										...
										<?php foreach ($imageid as $id) : ?>
											<a href="<?php usces_the_itemImageURL($id); ?>" <?php echo apply_filters('usces_itemimg_anchor_rel', NULL); ?>><?php usces_the_itemImage($id, 45, 35, $post); ?></a>
										<?php endforeach; ?>
							...
							<form action="<?php echo USCES_CART_URL; ?>" method="post">
								<div class="">
									<table class="">
										<thead>
											<tr>
												<th colspan="1"><?php _e('Title', 'usces'); ?></th>
												<?php if (usces_the_itemCprice('return') > 0) : ?>
													<th colspan="2">(<?php _e('List price', 'usces'); ?>)<?php _e('selling price', 'usces'); ?><?php usces_guid_tax(); ?></th>
												<?php else : ?>
													<th colspan="2" rowspan="2"><?php _e('selling price', 'usces'); ?><?php usces_guid_tax(); ?></th>
												<?php endif; ?>
											</tr>
											<tr>
												<th class=""><?php _e('Quantity', 'usces'); ?></th>
											</tr>
										</thead>
										<tbody>
											<?php do { ?>
												<tr>
													<td colspan="1" class="skudisp "><?php usces_the_itemSkuDisp(); ?>
														<?php if (usces_is_options()) : ?>
															<table class='item_option'>
																<caption><?php _e('Please appoint an option.', 'usces'); ?></caption>
																<?php while (usces_have_options()) : ?>
